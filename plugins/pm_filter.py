...
async def give_filter(client, message):
    ...
    if k == False:
        ...
        if 2 < len(message.text) < 100:
            files, offset, total_results = await get_search_results(message.text.lower(), offset=0, filter=True)
            if not files:
...
async def auto_filter(client, msg, q_id: str):
    ...
    k = 1
    for file in files:
        out += f"{k}. [{files.caption} [{get_size(file.file_size)}]](https://t.me/{temp.U_NAME}?start={pre}_{file.file_id})\n\n"
        k += 1
    btn = []
    if offset != "":
